...
## Load  the data
The data are stored as a `MultiAssayExperiment` object (see cheatsheet provided in the shared folder `Guidelines`, and this [link](https://bioconductor.org/packages/devel/bioc/vignettes/MultiAssayExperiment/inst/doc/MultiAssayExperiment.html)) to handle and sample match multi-omics data more efficiently. 
...
We required a minimum of 3 calls in the region followed by a stringent cell detection filtering of ~ 60% (500 cells out of 826 matching ones) for acc data and ~50% (400/826 cells) for met data which on average had less coverage. These selected thresholds can be changed. 
